...
class ticket_input(osv.osv):
    ...
    _columns = {
        ...
        'p_pauta': fields.float('Precio unitario de la pauta', digits=(16,3)),
...
#    def _check_estado(self,cr,uid,ids):
#        for estado in self.browse(cr, uid, ids):
#            if '' in estado.co_estado: return False 
#        return True
#    _constraints = [(_check_estado, 'Error de carga de datos archivo ticket SGC !', ['name'])]
...
#                                readonly=True),
    ...
    def validar(self, cr, uid, ids,context=None):
        ...
        if resulterr:
            ...
            cr.commit()
            cr.execute( "SELECT distinct t1.co_canilla ,t1.no_canilla, t1.ti_documento, t1.nu_documento  FROM ticket_trans t1 where not EXISTS (SELECT 1 FROM res_partner where co_canilla = t1.co_canilla and customer = True ) and co_estado = 'importado'  ")
            canilla = cr.fetchall()
            for row in canilla:
                canilla_reg = {
                'co_canilla': row[0],
                ...
                if res_id == 0:
                    cr.rollback()
                    raise osv.except_osv(_('Mensaje!'), 'error crear Canilla')
                else:
                    cr.commit()
            ...
            for row2 in producto:
                if row2[0] != mco_producto:
                    ...
                    if product_template_id == 0:
                        cr.rollback()
                        raise osv.except_osv(_('Mensaje!'), 'error crear Producto')
                    else:
                        cr.commit()
                mco_producto = row2[0]
    ...
    def preliminar(self, cr, uid, ids,context=None):
        ...
        if resulterr:
            ...
            if resulterr:
                ...
                cr.commit()
###  facturacion de registros del POS
#                cr.execute(""" select  count(*) from  pos_order ord where  note is null or note = '' """)
#                flg_proceso_odoo  = cr.fetchone()
#                if flg_proceso_odoo > 0:
#                    cr.execute("select fnc_insert_comprobante_odoo(%s)" , (idCiclo,))
#                    cr.execute(""" select distinct codigo, descripcion from tmp_err_ticket """)
#                    resulterr = cr.fetchall()
#                    if resulterr:
#                        cr.rollback()
